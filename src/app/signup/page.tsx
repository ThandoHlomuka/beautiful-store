...
export default function SignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (result.success) {
      logUpdate('success', 'Auth', `New user signed up: ${email}`);
      router.push('/account');
    } else {
      setError(result.error || 'Signup failed');
      logUpdate('warning', 'Auth', `Signup failed: ${email} - ${result.error}`);
    }
    setLoading(false);
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="absolute top-1/4 -right-20 w-80 h-80 bg-gradient-to-br from-blue-500/20 to-purple-500/20 rounded-full blur-3xl" />
      <div className="absolute -bottom-20 -left-20 w-80 h-80 bg-gradient-to-br from-pink-500/20 to-orange-500/20 rounded-full blur-3xl" />
      ...
      <div className="relative bg-white/80 dark:bg-gray-900/80 backdrop-blur-lg rounded-3xl p-8 w-full max-w-md border border-white/20 shadow-2xl">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-500 dark:text-gray-400 mt-2">Join Metra Marketplace today</p>
        </div>
